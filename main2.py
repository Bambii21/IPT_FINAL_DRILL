...
def verify_password(username, password):
    return username == "bakery" and password == "77777"
...
@auth.login_required
def get_customer():
    data = data_fetch("SELECT * FROM batches")
    return format_response(data)

@app.route("/batches/<int:id>", methods=["GET"])
@auth.login_required
def get_customer_by_id(id):
    data = data_fetch(f"SELECT * FROM batches WHERE batch_id = {id}")
    return format_response(data)

@app.route("/batches/<int:id>/ingredients", methods=["GET"])
@auth.login_required
def get_batches_by_batch_id(id):
    data = data_fetch(f"""
       SELECT batches.batch_id, ingredients.ingredient_id 
        FROM barches
        INNER JOIN ingredients
        ON batches.batch_id = ingredients.ingredient_id 
        WHERE batches.=batch_id = {id}""")
    response_data = {"batch_id": id, "count": len(data), "rate": data}
...
def add_batches():
    ...
    if not all([batch_id, Last_name, First_name, Contact_No, Email, Location, Password]):
        abort(400, description="Missing required fields in JSON data.")
        
    cur.execute(
        """ INSERT INTO customer (batch_id, Last_name, First_name, Contact_No, Email, Location, Password) 
